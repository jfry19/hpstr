...
bool VertexAnaProcessor::process(IEvent* ievent) { 
    ...
    for (auto region : _regions ) {
	    ...
	    if (!_reg_vtx_selectors[region]->passCutGt("uncVtxZ_gt",vtx->getZ(),weight))
            ...
            //L2 requirement for electron
            if (!_reg_vtx_selectors[region]->passCutEq("L2EleReq_eq",(int)(foundL2ele),weight))
                continue;

            //L1 L2 generic
            if (!_reg_vtx_selectors[region]->passCutEq("L1L2Requirement_eq",(int)((foundL1ele&&foundL2pos)||(foundL2ele&&foundL1pos)),weight))
                continue;

            //L1 ele L2 pos requirement
            if (!_reg_vtx_selectors[region]->passCutEq("L1EleReqL2PosReq_eq",(int)(foundL1ele&&foundL2pos),weight))
                continue;

            //L2 ele L1 pos requirement
            if(!_req_vtx_selectors[region]->passCutEq("L2EleReqL1PosReq_eq",(int)(foundL2ele&&foundL1pos),weight))
                continue;
